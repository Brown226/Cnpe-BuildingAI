/**
 * 文件引用 token 解析与格式化（对齐 Kun lib/composer-file-references 完整移植）。
 * 纯函数，无 React 依赖：处理工作区文件 @ 提及的 token（普通/引号/目录）、
 * 光标定位、替换/删除、过滤排序与上下文 prompt 构建。
 */
...
const MAX_COMPOSER_FILE_REFERENCE_BYTES = 16 * 1024;
...
export function parseComposerFileReferenceDragData(
  ...
  if (!raw || raw.length > MAX_COMPOSER_FILE_REFERENCE_BYTES) return null;
  if (new TextEncoder().encode(raw).byteLength > MAX_COMPOSER_FILE_REFERENCE_BYTES) return null;
  try {
