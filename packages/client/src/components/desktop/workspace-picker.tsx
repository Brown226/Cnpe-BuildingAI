/**
 * Composer/头部工作区切换器（复刻 Kun WorkspaceProjectPicker）：
 * 当前项 + 记忆列表（>5 出搜索框）+ 底部"添加文件夹"（系统目录框）。
 * 仅桌面环境渲染。
 */
import { ChevronDown, FolderOpen, FolderPlus, Search, Settings2 } from "lucide-react";
    ...
    return (
        <div ref={rootRef} className="relative">
            <button
                type="button"
                onClick={() => setOpen((v) => !v)}
                className="border-input bg-background hover:bg-accent flex h-8 max-w-56 items-center gap-1.5 rounded-md border px-2 text-sm transition-colors"
            ...
            {open && (
                <div className="bg-popover text-popover-foreground absolute top-full left-0 z-50 mt-1.5 w-80 overflow-hidden rounded-lg border shadow-md">
                    {showSearch && (
                    ...
                    <div className="border-t py-1">
                        ...
                        <button
                            ...
                            onClick={() => {
                                setOpen(false);
                                toast.info("左下角 ⚙ 可管理权限模式与白名单");
                            }}
                            className="hover:bg-accent text-muted-foreground flex w-full items-center gap-2.5 px-3 py-2 text-left text-sm"
                        >
