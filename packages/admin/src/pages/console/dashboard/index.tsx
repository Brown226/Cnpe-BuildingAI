...
const DashboardIndexPage = () => {
  ...
  // 企业免费模式（SERVER_BILLING_ENABLED !== "true"）下不展示订单/收入相关商业数据
  ...
  const userTrendCard = (
    <AreaChartCard
      ...
      className={billingEnabled ? "lg:col-span-2" : undefined}
      onTimeRangeChange={handleUserTimeRangeChange}
    />
  );

  const tokenRankingCard = (
    <DataCard
      title="Token使用排行"
              ...
              ? data?.tokenUsage.byModel.map((item, index) => (
                  <div className="flex items-center gap-2" key={item.modelId}>
                    ...
                    <div className="ml-auto text-right">
                      ...
                      <div className="text-muted-foreground text-xs">{item.conversations}次</div>
                    </div>
                  </div>
                ))
              ...
              data.extension.usageRanking.map((item, index) => (
                <div className="flex items-center gap-2" key={item.extensionId}>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-muted-foreground text-xs">应用ID: {item.extensionId}</div>
                  </div>
                  <div className="ml-auto text-right">
  ...
  return (
    <PageContainer>
      <div className="flex flex-col gap-4">
        <div
          className={`grid grid-cols-1 gap-4 ${billingEnabled ? "md:grid-cols-2 lg:grid-cols-3" : "md:grid-cols-2"}`}
        >
          {billingEnabled && (
            <DataCard
              title="订单统计"
        ...
        {billingEnabled ? (
          <>
            ...
            <div className="grid min-h-80 grid-cols-1 gap-4 lg:grid-cols-3">
              ...
              />
              {extensionRankingCard}
            </div>
          </>
        ) : (
          <div className="grid min-h-80 grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            {userTrendCard}
            {tokenRankingCard}
            {extensionRankingCard}
          </div>
